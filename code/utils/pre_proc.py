...
from utils.draw_gaussian import *


def processing_test(image, input_h, input_w):
    """
        将图片转化为固定大小的尺度，之后将其转化为channel_first的tensor类型
    """
...
def generate_ground_truth_hm(image,
                             pts_2,
                             image_h,
                             image_w,
                             img_id):
    num_landmarks = pts_2.shape[0]
    hm = np.zeros((num_landmarks, image_h, image_w), dtype=np.float32)
    ...
    for k in range(num_landmarks):
        ...
        draw_umich_gaussian(hm[0, :, :], pts_2[k], radius=3)
    return image, hm, pts_2
