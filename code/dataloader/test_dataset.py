...
class TestDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', num=None, num_class=7, transform=None, cv=1):
        ...
        liu_test1_label_file = "all_test_annotation/liu_test1"
        liu_test2_label_file = "all_test_annotation/liu_test2"

        wang_test1_label_file = "all_test_annotation/wang_test1"
        wang_test2_label_file = "all_test_annotation/wang_test2"

        self.liu_test1 = self.get_annotation(liu_test1_label_file)

        self.liu_test2 = self.get_annotation(liu_test2_label_file)

        self.wang_test1 = self.get_annotation(wang_test1_label_file)

        self.wang_test2 = self.get_annotation(wang_test2_label_file)

    def get_annotation(self, file_name):
        with open(os.path.join(self._base_dir, file_name), 'rb') as f:
    ...
    def get_data(self, annotation, case):
        label, img_type = annotation[str(case)]
        test_label = label[:self.num_class]
        return test_label, img_type
    ...
    def __getitem__(self, idx):
        ...
        liu_test1_label, _ = self.get_data(self.liu_test1, case)
        liu_test2_label, _ = self.get_data(self.liu_test2, case)
        mean_liu_label = (liu_test1_label + liu_test2_label) / 2

        wang_test1_label, _ = self.get_data(self.wang_test1, case)
        wang_test2_label, _ = self.get_data(self.wang_test2, case)
        mean_wang_label = (wang_test1_label + wang_test2_label) / 2
        train_label = (mean_liu_label + mean_wang_label) / 2

        space = self.space[case]

        h, w = image.shape
        kpts_wrapped = sld.KeyPoints(train_label, h, w)
